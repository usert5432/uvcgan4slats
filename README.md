...
For the generator pretraining and image-to-image translation training,
we will use `SLATS` scripts as examples. We recommend the following appraoch,
when adapting `UVCGAN` for you needs. Start with one of the provided example
scripts. Make minimal modifications to make it work for your problem. Once it
is working -- further customize the model configuration to achieve the best
results.
...
There are some considerations, depending on the format of your dataset.

### 0.1 Natural images
  This repository is primarily focused on scientific datasets.
  If your dataset is made of natural images in common formats
  (`jepg`, `png`, `webp` [etc][image_ext]), you may find it useful to take
  one of the [`UVCGAN`][uvcgan_repo] or [`UVCGANv2`][uvcgan2_repo] training
  scripts as a staring point.
